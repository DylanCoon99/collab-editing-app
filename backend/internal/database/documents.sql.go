...
const getOwnerIDByDocumentID = `-- name: GetOwnerIDByDocumentID :one
SELECT owner_id
FROM documents
WHERE id = $1
`

func (q *Queries) GetOwnerIDByDocumentID(ctx context.Context, id uuid.UUID) (uuid.NullUUID, error) {
	row := q.db.QueryRowContext(ctx, getOwnerIDByDocumentID, id)
	var owner_id uuid.NullUUID
